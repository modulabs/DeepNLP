...
def main(self):
    ...
    return
    # 훈련셋 인코딩 만드는 부분이다.
    inputTrainEnc, inputTrainEncLength = data.encProcessing(xTrain[:100], char2idx)
    # inputTrainEnc, inputTrainEncLength = data.encProcessing(xTrain, char2idx)
    # 훈련셋 디코딩 입력 부분 만드는 부분이다.
    outputTrainDec, outputTrainDecLength = data.decOutputProcessing(yTrain[:100], char2idx)
    # outputTrainDec, outputTrainDecLength = data.decOutputProcessing(yTrain, char2idx)
    # 훈련셋 디코딩 출력 부분 만드는 부분이다.
    targetTrainDec = data.decTargetProcessing(yTrain[:100], char2idx)
    ...
    inputTestEnc, inputTestEncLength = data.encProcessing(xTest[:10], char2idx)
    outputTestDec, outputTestDecLength = data.decOutputProcessing(yTest[:10], char2idx)
    targetTestDec = data.decTargetProcessing(yTest[:10], char2idx)
